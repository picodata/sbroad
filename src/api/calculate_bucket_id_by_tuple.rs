...
/// Calculate the target bucket by a Lua tuple or Lua array
#[no_mangle]
pub extern "C" fn calculate_bucket_id_by_tuple(ctx: FunctionCtx, args: FunctionArgs) -> c_int {
    ...
    COORDINATOR_ENGINE.with(|e| {
        ...
        match engine.extract_sharding_keys_from_tuple(params.space, &params.rec) {
            Ok(tuple) => {
